...
class SelfOptimizationLoop:
    ...
    async def run_full_optimization(self) -> Dict[str, Any]:
        ...
        try:
            ...
            memories = await self.storage.search_memories(
                query="", limit=1000, score_threshold=0.0  # 空查询获取所有
            )
    ...
    async def _optimize_single_memory(
        self, memory: Dict[str, Any], stats: Dict[str, Any]
    ):
        """优化单个记忆"""
    ...
    async def _upgrade_to_qmd(
        self, memory_id: str, text: str, metadata: Dict[str, Any], score: float
    ):
        """将记忆升级到QMD文件系统"""
        try:
            # 1. 创建QMD目录结构
    ...
    def _format_qmd_content(
        self, text: str, metadata: Dict[str, Any], score: float
    ) -> str:
        """格式化QMD文件内容"""
        category = metadata.get("category", "other")
...
"""
    ...
    async def _demote_memory(
        self, memory_id: str, metadata: Dict[str, Any], score: float
    ):
        """降权记忆（降低重要性）"""
