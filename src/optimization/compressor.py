...
class MemoryCompressor:
    ...
    def __init__(self, min_length: int = 100, max_ratio: float = 0.5):
        ...
        logger.info(
            f"记忆压缩器初始化完成 (min_length={min_length}, max_ratio={max_ratio})"
        )
    ...
    def compress(self, text: str, aggressive: bool = False) -> CompressionResult:
        ...
        try:
            ...
            logger.debug(
                f"文本压缩完成: {len(original_text)} → {len(text)} 字符, 压缩率: {compression_ratio:.2f}"
            )
            return result
    ...
    def batch_compress(
        self, texts: List[str], aggressive: bool = False
...
def create_compressor(
    min_length: int = 100, max_ratio: float = 0.5
) -> MemoryCompressor:
