...
class TokenOptimizer:
    ...
    def __init__(self, config: Optional[TokenOptimizationConfig] = None):
        ...
        self.compressor = EfficientCompressor(
    ...
    async def optimize_context(
        ...
        logger.info(
            f"开始Token优化: 查询='{query[:50]}...', 记忆数={len(memories)}, 目标Token数={max_tokens}"
        )
        ...
        if token_count > max_tokens:
            optimized_context = await self._secondary_compression(
        ...
        logger.info(
            f"Token优化完成: 原始记忆={len(memories)}, 选择={len(selected_memories)}, "
    ...
    async def _calculate_memory_score(
        self, memory: Dict[str, Any], query: str
    ) -> float:
        """计算记忆优化分数"""
        score = 0.0
    ...
    async def _select_memories(
        self, scored_memories: List[Dict[str, Any]], max_tokens: int
    ) -> List[Dict[str, Any]]:
        """智能选择记忆 - 最大化Token节省"""
        selected = []
        current_tokens = 0
    ...
    async def _secondary_compression(self, context: str, max_tokens: int) -> str:
        ...
        # 选择性保留记忆
        memory_lines = [
            line for line in lines if line.startswith(("1.", "2.", "3.", "4.", "5."))
        ]
    ...
    def _update_stats(
        ...
        # 计算压缩率
        compression_ratio = (
            final_token_count / original_tokens if original_tokens > 0 else 1.0
        )
        ...
        new_avg = (
            current_avg * (self.optimization_stats["total_optimizations"] - 1)
            + compression_ratio
