...
class HierarchicalMemorySystem:
    ...
    async def process_memory(self, text: str, metadata: Dict[str, Any] = None) -> str:
        ...
        logger.info(f"记忆处理完成: ID={memory_id[:8]}, 层级={target_layer}, 评分={score:.2f}")
    ...
    def _compress_memory(self, text: str, layer: str) -> str:
        ...
        if len(lines) >= 3:
            ...
            compressed = text[:50] + "..." + text[mid_point:mid_point+50] + "..." + text[-50:]
    ...
    async def _store_memory(self, text: str, original_text: str, embedding: List[float],
                           score: float, layer: str, metadata: Dict[str, Any]) -> str:
        """存储记忆"""
        memory_id = f"mem_{datetime.now().strftime('%Y%m%d_%H%M%S')}_{hash(text) % 10000:04d}"
        ...
        if len(self.memory_cache[layer]) > self.layers[layer].capacity:
            self.memory_cache[layer] = self.memory_cache[layer][-self.layers[layer].capacity:]
    ...
    async def retrieve_relevant_memories(self, query: str, limit: int = 10) -> List[Dict[str, Any]]:
        ...
        logger.info(f"检索完成: 查询='{query[:50]}...', 返回{len(selected_memories)}条记忆")
        
        return selected_memories
    
    async def _retrieve_from_layer(self, query_embedding: List[float], 
                                  layer: str, limit: int) -> List[Dict[str, Any]]:
        """从指定层级检索记忆"""
        layer_dir = self.storage_dir / layer
    ...
    def _select_optimal_memories(self, memories: List[Dict[str, Any]], 
    ...
    async def get_memory_stats(self) -> Dict[str, Any]:
        ...
        if total_original_length > 0:
            stats["average_compression_ratio"] = total_compressed_length / total_original_length
