...
class MemoryRetrieval:
    ...
    def search(
        ...
        try:
            ...
            logger.info(f"搜索完成: 查询='{query[:50]}...', 找到 {len(final_results)} 条相关记忆")
    ...
    def _build_search_filters(self, user_filters: Optional[Dict[str, Any]]) -> Optional[qdrant_models.Filter]:
    ...
    def _passes_filter(self, memory: MemoryRecord, filters: qdrant_models.Filter) -> bool:
    ...
    def _process_results(
        ...
        for memory, similarity in search_results:
            ...
            explanation = ""
            if explain:
                explanation = self._generate_explanation(memory, similarity, relevance, query)
    ...
    def _calculate_relevance(self, memory: MemoryRecord, similarity: float, query: str) -> float:
