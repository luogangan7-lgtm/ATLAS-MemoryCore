/**
 * ATLAS Memory v11.0.0 — 自主演化知识系统（L0-L3 · source_type · TTL · 意图检索 · MCP Server:8766）
 *
 * 架构（四层，商业级）：
...
async function qdrantSearch(vector, { limit = 5, category, domain, minScore = SCORE_MIN, source_type, platform } = {}) {
  const body = { vector, limit, with_payload: true, score_threshold: minScore };
  // 永远过滤 superseded + archived + 已过期（expires_at < now）
  const now = new Date().toISOString();
  const mustNot = [
    { key: 'status', match: { value: 'superseded' } },
    { key: 'status', match: { value: 'archived' } },
    { key: 'expires_at', range: { lt: now } },  // null 不参与 range 比较，永久记录不受影响
  ];
  const must = [];
...
async function mcpExecute(toolName, args) {
  switch (toolName) {
    ...
    case 'atlas_recall': {
      ...
      return applyTimeDecay(hits, 'relevant').map(h => ({
...
export function register(api) {
  ...
  api.registerTool(() => ({
    name: 'atlas_store',
    description:
      '将重要信息存入语义记忆库（v11）。自动检测 source_type 和 TTL，冲突检测自动合并/替换，相似度≥0.92去重跳过。',
    parameters: {
      type: 'object', required: ['content'],
      properties: {
        content:           { type: 'string',  description: '要存储的内容（5-500字）' },
        ...
        tags:              { type: 'array',   items: { type: 'string' }, default: [] },
        source_type:       { type: 'string',  enum: ['trading', 'news', 'social', 'course', 'chat', 'platform_rule', 'process', 'unknown'],
                             description: '信息源类型（不填自动检测）。trading=5分钟TTL, news=3天, social=7天, platform_rule=90天, course/process=永久' },
        knowledge_purpose: { type: 'string',  enum: ['understanding', 'production', 'process'],
                             description: '知识颗粒度方向（不填自动检测）。understanding=深度理解, production=模板可直接用, process=流程SOP' },
        platform:          { type: 'string',  description: '适用平台，多个逗号分隔，如"抖音,小红书"' },
        expires_at:        { type: 'string',  description: 'ISO8601过期时间（留空由source_type自动计算）' },
      },
    },
    execute: async (_callId, params) => {
      const { content, category = 'work', importance = 'medium', memory_type = 'fact',
              tags = [], source_type, knowledge_purpose, platform, expires_at } = params ?? {};
      if (!content?.trim()) return jsonResult({ error: 'content 不能为空' });
      const detectedSourceType = source_type ?? detectSourceType(content, '', tags);
      const result = await storeMemory({
        content, category, importance, memory_type, tags, source: 'manual',
        source_type: detectedSourceType,
        knowledge_purpose: knowledge_purpose ?? detectKnowledgePurpose(content, tags, detectedSourceType),
        platform: platform ?? null,
        expires_at: expires_at ?? calcTTLExpiry(detectedSourceType),
      });
      ...
      return jsonResult({ ok: true, id: result.id, category, importance, memory_type,
                          source_type: detectedSourceType, ttl: TTL_MAP[detectedSourceType] ?? 'permanent' });
    },
  }));
  ...
  api.registerTool(() => ({
    name: 'atlas_recall',
    description: '从语义记忆库检索相关知识（v11：意图感知 + source_type/platform 过滤 + TTL自动过滤）。',
    parameters: {
      type: 'object', required: ['query'],
      properties: {
        ...
        source_type: { type: 'string',  enum: ['trading', 'news', 'social', 'course', 'chat', 'platform_rule', 'process', 'unknown'],
                       description: '按信息源类型过滤，不填返回所有类型' },
        platform:    { type: 'string',  description: '按平台过滤，如"抖音"' },
        intent:      { type: 'string',  enum: ['relevant', 'latest'], default: 'relevant',
                       description: 'relevant=语义最相关(默认), latest=时间最新（热点/交易场景用）' },
      },
    },
    execute: async (_callId, params) => {
      const { query, limit = 5, category = 'any', min_score = SCORE_MIN,
              source_type, platform, intent = 'relevant' } = params ?? {};
      ...
      const hits    = await qdrantSearch(vector, { limit, category, minScore: min_score, source_type, platform });
      const sorted  = applyTimeDecay(hits, intent);
